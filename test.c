...
void signUpAdmin() {
    ...
    // Generate admin ID
    srand(time(NULL));
    sprintf(newAdmin.admin_id, "ADM-%04d", rand() % 10000);

    printf("Your Admin ID: %s\n", newAdmin.admin_id);
    ...
    fprintf(adminFile, "%s %s %s\n", newAdmin.admin_id, newAdmin.username, newAdmin.password);
    fclose(adminFile);
...
void loginAdmin() {
    ...
    int numAdmins = 0;
    while (fscanf(adminFile, "%s %s %s", admins[numAdmins].admin_id, admins[numAdmins].username, admins[numAdmins].password) == 3) {
        numAdmins++;
    }
    fclose(adminFile);
    ...
    for (int i = 0; i < numAdmins; i++) {
        if (strcmp(admins[i].admin_id, admin_id) == 0 && strcmp(admins[i].username, username) == 0 && strcmp(admins[i].password, password) == 0) {
            authenticated = 1;
            break;
        }
    }
    ...
    if (authenticated) {
        ...
        printf("Invalid admin ID, username, or password. Exiting program.\n");
        exit(EXIT_FAILURE);
    }
}
...
void addProduct() {
    ...
    strcpy(products[numProducts].product_id, product_id);
